...
class Config(context: Context)  {
    ...
    var stroboscopeFrequencyOn: Long
        ...
        set(stroboscopeFrequencyOn) = prefs?.setValueLong(STROBOSCOPE_FREQUENCY_ON, stroboscopeFrequencyOn)

    var stroboscopeFrequencyOff: Long
        get() = prefs.getValueLong(STROBOSCOPE_FREQUENCY_OFF, 1000L)
        set(stroboscopeFrequencyOff) = prefs?.setValueLong(STROBOSCOPE_FREQUENCY_OFF, stroboscopeFrequencyOff)


    var stroboscopeOnProgress: Int
        get() = prefs.getValueInt(STROBOSCOPE_PROGRESS_ON, 1000)
        set(stroboscopeOnFrequency) = prefs?.setValueInt(STROBOSCOPE_PROGRESS_ON, stroboscopeOnFrequency)

    var stroboscopeOffProgress: Int
        get() = prefs.getValueInt(STROBOSCOPE_PROGRESS_OFF, 1000)
        set(stroboscopeOffFrequency) = prefs?.setValueInt(STROBOSCOPE_PROGRESS_OFF, stroboscopeOffFrequency)
